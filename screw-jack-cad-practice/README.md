...
![Screw jack isometric render](renders/isometric-render.png)
...
- **`renders/`**: reference images, including a cross section view showing the thread engagement between the screw spindle and nut, an isometric view of the finished assembly, and an early detail study of the thread profile.

## About the design

A screw jack converts rotary motion (turning the tommy bar) into linear motion (raising the cup) via a threaded screw spindle running through a fixed nut. It's a classic exercise for practicing threaded fastener modeling, assembly mating, and section view analysis in CAD. The sectioned render shows the thread engagement and how the load path runs from the cup down through the spindle, nut, and body to the base.
